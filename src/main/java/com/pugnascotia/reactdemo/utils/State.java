...
public final class State {
    ...
    /**
     * Returns the request string, including the query fragment, so that it can be made available during server-side
    ...
    /**
     * Return a list of the current user's roles. If they are not authenticated then they will have "ROLE_ANONYMOUS".
     */
    private static Optional<List<String>> getRoles(HttpServletRequest request) {
        return getAuthentication(request).map(a -> Functions.map(a.getAuthorities(), GrantedAuthority::getAuthority));
    }

    /**
     * Getting the current authentication object ought to be easy, by getting a context with
     * {@link SecurityContextHolder#getContext()}, then calling {@link SecurityContext#getAuthentication()}. However
     * there are circumstances where that doesn't work reliably, such as when handling exceptions. This method makes
     * several attempts to get an {@link Authentication} instance.
     */
    private static Optional<Authentication> getAuthentication(HttpServletRequest request) {
        ...
        if (authentication == null) {
            ...
            SecurityContext securityContext = (SecurityContext) requestAttributes
                    .getAttribute("SPRING_SECURITY_CONTEXT", RequestAttributes.SCOPE_SESSION);
            if (securityContext == null) {
                securityContext = (SecurityContext) requestAttributes.getAttribute("SPRING_SECURITY_CONTEXT",
